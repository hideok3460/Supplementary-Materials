...
This supplementary note contains the extended controlled-$X$ derivation that was formerly included as Appendix B of the main manuscript.  The title and notation in this version are synchronized with the current main manuscript; the hardware tomography and CP-TNI reconstruction are documented separately in Supplementary File S2.  It is separated from the article because the result is supporting notation rather than a central contribution of the channel-equivalence theorem.  The derivation fixes the computational-basis convention, gives a compact Dirac-expanded representation of $\CX_{i,j,q}$, embeds the operator explicitly in an arbitrary $q$-qubit tensor product, and checks the two- and three-qubit conventions used by the manuscript.  The notation is motivated by the Dirac extension of CNOT gates discussed by Yamanashi, Ozawa, and Okazaki~\cite{Yamanashi2023}.
